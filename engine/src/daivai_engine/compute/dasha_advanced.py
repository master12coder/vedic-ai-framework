...
class DashaSandhi(BaseModel):
    """Junction period between two Mahadashas."""
...
def compute_dasha_sandhi(mahadashas: list[DashaPeriod]) -> list[DashaSandhi]:
    ...
    for i in range(len(mahadashas) - 1):
        ...
        nature = _classify_transition(ending.lord, starting.lord)
        ...
        results.append(
            DashaSandhi(
                ...
                nature=nature,
                description=f"{ending.lord} → {starting.lord} transition ({nature})",
            )
        )
...
def _classify_transition(ending: str, starting: str) -> str:
    """Classify the nature of a dasha transition."""
    e_benefic = ending in _NATURAL_BENEFICS
    s_benefic = starting in _NATURAL_BENEFICS
    if e_benefic and s_benefic:
        return "easy"
    if not e_benefic and not s_benefic:
        return "challenging"
