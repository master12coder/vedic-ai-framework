...
class TestKemadrumaBhanga:
    ...
    def test_kemadruma_uncancelled_when_all_rules_fail(self):
        ...
        assert (
            "Kemadruma Yoga (Uncancelled)" in names
            or "Kemadruma Bhanga (Multiple Rules)" in names
        )
...
class TestRajaYogaBhanga:
    def test_kendra_lord_with_rahu_detects_bhanga(self):
        ...
        """
        chart = make_chart(1, {  # Taurus lagna
            "Sun": (0, 12),
            "Moon": (4, 4),
            "Mars": (7, 7),
            "Mercury": (1, 1),
...
class TestConjunctionYogas:
    def test_mars_saturn_conjunction_detected(self):
        ...
        chart = make_chart(0, {
            "Sun": (0, 1), "Moon": (3, 4),
            "Mars": (8, 9), "Mercury": (0, 1),
            "Jupiter": (6, 7), "Venus": (0, 1),
    ...
    def test_mars_jupiter_conjunction_is_benefic(self):
        ...
        chart = make_chart(0, {
            "Sun": (0, 1), "Moon": (3, 4),
            "Mars": (6, 7), "Mercury": (0, 1),
            "Jupiter": (6, 7), "Venus": (0, 1),
            "Saturn": (0, 1), "Rahu": (2, 3), "Ketu": (8, 9),
        })
        results = detect_conjunction_yogas(chart)
    ...
    def test_no_conjunction_when_planets_in_different_signs(self):
        ...
        chart = make_chart(0, {
            "Sun": (0, 1), "Moon": (3, 4),
            "Mars": (0, 1), "Mercury": (1, 2),
            "Jupiter": (6, 7), "Venus": (9, 10),
            "Saturn": (4, 5), "Rahu": (2, 3), "Ketu": (8, 9),
        })
        results = detect_conjunction_yogas(chart)
...
class TestSunaphaAnaphaSpecific:
    ...
    def test_mars_in_12th_from_moon_gives_anapha(self):
        ...
        chart = make_chart(0, {
            ...
            "Mercury": (0, 1),
            "Jupiter": (0, 1), "Venus": (0, 1), "Saturn": (0, 1),
            "Rahu": (2, 3), "Ketu": (8, 9),
        })
        results = detect_sunapha_anapha_specific(chart)
        names = _yoga_names(results)
...
class TestNewYogaYamlCount:
    def test_total_yoga_definitions_exceed_284(self):
        ...
        assert len(defs) >= 284, (
            f"Expected ≥284 yoga definitions, got {len(defs)}"
        )
    ...
    def test_new_yogas_have_required_fields(self):
        ...
        new_keys = [
            "bandhana_yoga_1", "arishta_bhanga_1", "kemadruma_bhanga_rule1",
            "raj_yoga_bhanga_1", "balarishta_5", "daridra_yoga_11",
            "dhana_yoga_1_9", "sanyasa_yoga_4", "mars_saturn_conjunction",
            "musala_yoga", "vajra_yoga",
        ]
        for key in new_keys:
            if key not in defs:
